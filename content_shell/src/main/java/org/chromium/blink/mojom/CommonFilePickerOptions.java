package org.chromium.blink.mojom;

import java.nio.ByteBuffer;
import java.util.ArrayList;
import org.chromium.mojo.bindings.DataHeader;
import org.chromium.mojo.bindings.Decoder;
import org.chromium.mojo.bindings.Encoder;
import org.chromium.mojo.bindings.Interface;
import org.chromium.mojo.bindings.Message;
import org.chromium.mojo.bindings.Struct;
/* loaded from: classes2.dex */
public final class CommonFilePickerOptions extends Struct {
    private static final DataHeader DEFAULT_STRUCT_INFO;
    private static final int STRUCT_SIZE = 32;
    private static final DataHeader[] VERSION_ARRAY;
    public String startingDirectoryId;
    public FileSystemAccessTransferToken startingDirectoryToken;
    public int wellKnownStartingDirectory;

    static {
        DataHeader[] dataHeaderArr = {new DataHeader(32, 0)};
        VERSION_ARRAY = dataHeaderArr;
        DEFAULT_STRUCT_INFO = dataHeaderArr[0];
    }

    private CommonFilePickerOptions(int version) {
        super(32, version);
    }

    public CommonFilePickerOptions() {
        this(0);
    }

    public static CommonFilePickerOptions deserialize(Message message) {
        return decode(new Decoder(message));
    }

    public static CommonFilePickerOptions deserialize(ByteBuffer data) {
        return deserialize(new Message(data, new ArrayList()));
    }

    public static CommonFilePickerOptions decode(Decoder decoder0) {
        if (decoder0 == null) {
            return null;
        }
        decoder0.increaseStackDepth();
        try {
            DataHeader mainDataHeader = decoder0.readAndValidateDataHeader(VERSION_ARRAY);
            int elementsOrVersion = mainDataHeader.elementsOrVersion;
            CommonFilePickerOptions result = new CommonFilePickerOptions(elementsOrVersion);
            result.startingDirectoryId = decoder0.readString(8, false);
            int readInt = decoder0.readInt(16);
            result.wellKnownStartingDirectory = readInt;
            WellKnownDirectory.validate(readInt);
            result.wellKnownStartingDirectory = WellKnownDirectory.toKnownValue(result.wellKnownStartingDirectory);
            result.startingDirectoryToken = (FileSystemAccessTransferToken) decoder0.readServiceInterface(20, true, FileSystemAccessTransferToken.MANAGER);
            return result;
        } finally {
            decoder0.decreaseStackDepth();
        }
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // org.chromium.mojo.bindings.Struct
    public final void encode(Encoder encoder) {
        Encoder encoder0 = encoder.getEncoderAtDataOffset(DEFAULT_STRUCT_INFO);
        encoder0.encode(this.startingDirectoryId, 8, false);
        encoder0.encode(this.wellKnownStartingDirectory, 16);
        encoder0.encode( this.startingDirectoryToken, 20, true,  FileSystemAccessTransferToken.MANAGER);
    }
}

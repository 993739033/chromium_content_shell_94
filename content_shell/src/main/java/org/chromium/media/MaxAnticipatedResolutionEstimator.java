...
public class MaxAnticipatedResolutionEstimator {
    ...
    public static Resolution getScreenResolution(MediaFormat format) {
        ...
        if (!format.getString(NdefMessageUtils.RECORD_TYPE_MIME).equals(MediaCodecUtil.MimeTypes.VIDEO_HEVC) && !format.getString(NdefMessageUtils.RECORD_TYPE_MIME).equals(MediaCodecUtil.MimeTypes.VIDEO_VP9)) {
            resolution.mWidth = Math.min(resolution.mWidth, (int) WebFeature.MEDIA_STREAM_CONSTRAINTS_VIDEO);
            resolution.mHeight = Math.min(resolution.mHeight, (int) WebFeature.AUDIO_NODE_DISCONNECT_FROM_AUDIO_NODE);
        }
        return resolution;
    }
    ...
    public static Resolution getNativeResolution() {
        if (Build.VERSION.SDK_INT == 28 && !isNvidiaShield() && is4kVpxSupported()) {
            Log.d(TAG, "Assuming 4K display capabilities because we can decode VP9 4K video.", new Object[0]);
            return new Resolution(3840, SCREEN_HEIGHT_4K);
        }
        Context context = ContextUtils.getApplicationContext();
        DisplayManager displayManager = (DisplayManager) context.getSystemService("display");
        DisplayCompat.ModeCompat[] supportedModes = DisplayCompat.getSupportedModes(context, displayManager.getDisplay(0));
    ...
    private static boolean is4kVpxSupported() {
        return ScreenResolutionUtil.isResolutionSupportedForType(MediaCodecUtil.MimeTypes.VIDEO_VP9, new Size(3840, SCREEN_HEIGHT_4K));
    }
}

    ...
    protected int dequeueOutputBufferInternal(MediaCodec.BufferInfo info, long timeoutUs) {
        ...
        try {
            ...
            if (indexOrStatus >= 0) {
                ...
                if (isConfigFrame) {
                    ...
                    while (true) {
                        ...
                        spsData.append(Integer.toHexString(this.mConfigData.get(i) & 255));
                        spsData.append(" ");
                        i++;
                    }
            ...
            if (indexOrStatus >= 0) {
                ...
                if (isKeyFrame) {
                    Log.d(TAG, "Key frame generated", new Object[0]);
                }
                if (isKeyFrame && (byteBuffer = this.mConfigData) != null) {
                    Log.d(TAG, "Appending config frame of size %d to output buffer with size %d", Integer.valueOf(byteBuffer.capacity()), Integer.valueOf(info.size));
